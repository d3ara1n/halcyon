...
调用方提供的相对 Timeout 在 RPC 入口一次转换为绝对单调 deadline，覆盖本地投递背压、回复等待与应答接受的完整调用；各阶段只消费剩余预算，不能按阶段重新开始同一相对时长。Timeout 只表示调用方停止等待，不撤销已入服务队列或正在执行的请求。若请求已投递，超时后的业务结果是“是否执行未知”；通用库不得自动重试有副作用调用。
