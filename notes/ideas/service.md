...
同一提供者的文件域与发现域可以共享 FAL 消息入口，但各自的根授权和节点归属仍划定互不穿越的领域；请求须由实际发送授权定位根，再由目标领域执行路径与权限验证。共享入口不合并注册状态与文件状态，也不允许跨域 Move。它同时共享接收、封口和执行资源，不承诺某个领域在另一领域拥塞或退出时独立可用；需要这种保障时应单独设计准入与端点边界。
...
建立期限由注册状态权威与 Ready 共同排序：只有当前仍为 Starting 且期限已到的实例才能因建立超时撤出；在期限内提交的 Ready 不因观察者稍后被调度而倒退。观察任务的时钟唤醒只是请求检查，不是第二份业务状态。发布、撤出和到期从同一次权威提交产生记录/目录代次与失效效果，通知发送可以异步推进但不能在请求层另行猜测提交结果。
