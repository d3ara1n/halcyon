...
通用执行机制可由普通应用、协议库及服务共同使用，不拥有服务领域状态。libsrv 拥有服务的声明、提供、使用与生命周期，并通过公共执行、记账和各领域能力组成服务框架；使用服务的客户端不因使用该领域库而承担发布或管理职责。

libsrv 消费用户态启动契约中的服务 owner 与依赖，参数、namespace 和进程构造各自复用所属领域的接口，不重新定义它们。服务不靠运行后的隐式消息补充启动 authority。activate、drain、reload 是独立控制协议，不与 launch 混称 STARTUP。
...
独立的通用记账库提供账户、额度、预留与真实释放后的退款；领域资源类别由领域定义，通用执行与记账均不枚举文件节点、目录 grant、Watch 或服务记录。资源释放只有在使等待者可以继续时才产生任务唤醒；最后引用消散产生了退休工作，则必须主动唤醒其执行者。
