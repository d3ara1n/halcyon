...
同步 Caller 的有限 `timeout_ms` 当前只用于请求投递成功后的 ReplyPort 等待；之前的 `send_blocking` 在 MailboxFull 时无限等待，因此它还不是完整调用 deadline。用户态尚无公共单调时钟，不能通过重复相对 WaitMany 正确拼出总期限；该缺口现已由 [`单调时间与 RPC 全调用期限`](../../plans/archived/todo-2026-09-monotonic-time-rpc-deadline.md) 完成，RPC 的完整阶段策略由执行前置继续承接。
