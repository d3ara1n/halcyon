...
- `MemoryUnmap = 0x51` 对普通 AddressSpace owner 执行严格全覆盖与精确区间切割；空洞、guard/mapping 种类违约或 lease owner 拒绝整笔请求。Unmap 与 Protect 都是两段式：Validate 在 AddressSpace 锁内定几何并报告所需 `WritePermit` 多重集（含 W 的 object view 被部分撤销或降权时，存活片段是新铸造区域、各需一枚新 permit），permit 在 AddressSpace 锁外向来源对象取得后重入 Reserve——对象状态锁秩低于 AddressSpace。Unmap 的页表投影按完整连续请求聚合成一个 Remove，Guard 区间自然为空。匿名 `OwnedBacking` 以 `reserved_extent_growth` 统计全部在途事务尚未兑现的最坏 Vec 增长；Remote ack 后每步只切下一个 owner，替换 extent 时把旧 metadata permit 回收到同一 continuation，只有双 survivor 的净增长才永久消费预付 permit。物理 owner 随后在 AddressSpace 锁外归还；Protect 的旧视图仍被新 ledger 完整覆盖时以 `BackingRetire::Retain` 跳过 backing 释放。
