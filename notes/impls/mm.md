...
`Process.space` 是稳定 `AddressSpace` 外壳：单调不复用的 identity、translation/instruction epoch 和内部 `AddressSpaceState::{Unbound, Bound}` 锁分离。Unbound 不持 ledger、页表或页额度；一次性 ProcessBindMemory 发布的 Bound 状态持完整 `PoolBinding`、`MemorySpace` ledger 与 `TableTree`。Bind 在锁外从绑定 Pool 取得 funded root，并将唯一 root owner 移交给 `TableTree`；`PoolBinding` 只表达绑定 authority、资源来源与 metadata sponsor，不与树重复持有 root。Bound 内的 ledger 是全部用户区域的 VA 真值；anonymous 区域由 `OwnedBacking(BackingId + logical page offset + affine extent owner)` 持有，对象区域由地址空间持有的 per-object view owner（对象 core 强引用 + `ObjectViewPermit`）引用对象的固定 backing，PTE 只是投影；object-owned lease（当前只有 Tunnel）另持 `ObjectMappingLease` 作为撤销凭据，进程自有 view 以账本区间与 AddressSpace authority 为真值，不铸造第二份身份。对象映射按对象几何投影出的物理 span 序列逐段准备 translation，表页按段独立预算，与匿名多 extent 映射同形；单页 view 退化为长度为一，Tunnel 即此情形。view 越界以 `ObjectViewAuthorization` 携带的对象固定长度为准。Remote Call 只引用外壳身份与 epoch。Running 变更在 Commit 前快照 lifecycle execution gate，准备 planner/backing 或 WritePermit/PTE/WaitContext、三类 metadata permit、deferred-work 槽和全部 Remote 目标槽；`publish_epochs` 先以 CAS 确认 translation/instruction epoch 均可前进，任一达到最大值就在 ledger/PTE 修改前返回耗尽错误，不写零也不污染旧 epoch。Commit 在 `ADDRESS_SPACE → LIFECYCLE` 下复检 active sequence、发布 ledger/PTE/epoch 并登记 mandatory operation，锁外敲门铃。最后 ack 只进入 Retiring 并发布 work debt；owner hart 在后续 trap/scheduler 安全点逐批退休资源，最终 Complete 才解除外部义务。
