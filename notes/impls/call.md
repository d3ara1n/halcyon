...
真实 MemoryChange 消费者包括 Running `Extend`、TunnelCreate/Attach 和显式 Endpoint HandleClose。它们在 Commit 前准备 planner/backing 或 WritePermit/PTE reservation、无 Thread 的 WaitContext 与 Remote slots；Commit 同步发布 PTE/ledger/epoch 并登记 lifecycle mandatory operation；最后 ack 的 completion 推进 `PublishedChange → Synchronize → Retire → Complete`，再解除 mandatory 屏障并交付已承诺结果。Tunnel Close 的 retire sink 在确认后归还 WritePermit、发布 CLOSED/PEER_CLOSED；发起线程若同时终止只放弃回复权，不撤销事务，REAPABLE 严格等待 mandatory operation 归零。
