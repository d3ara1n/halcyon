...
> 状态：独立待实施的结构收束，不是运输/服务执行前置的整体开工阻塞。审视基线为 `master@5d406a4...task/fal-service-capabilities@bf48cab`。ProcessDrain 的职责与 REAPABLE 触发已澄清，保留管理者有界推进；不以全体进程持续轮询、缺乏预算激励或必须自动回收作为改造依据。
>
