...
- 基线分支 `task/fal-service-capabilities`，HEAD `84eeed6`；F1–F3c 的代码、文档和装配仍共同位于未提交工作树。必须保留全部既有新增、修改和删除，不通过回退或重建 checkout 丢弃迁移成果。
...
本次仅完成原则落档、唯一立项与导航调整；未改 Cargo、Rust 源码或目录，未运行代码测试与 QEMU。下一步从 L0 审计开始，先确定公共记账和执行的真实知识边界、目标包/类型图及受影响计划，再进行 L1。FAL F3d 在本专题完成前不开始业务实施；其业务状态机、注册 authority、发现快照和失败路径仍由 FAL 总计划唯一拥有。
