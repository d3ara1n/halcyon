...
| [`todo-2026-09-memory-object-unification.md`](todo-2026-09-memory-object-unification.md) | **当前主线**：切片 7 公共 MemoryObject。步骤 1–3 已完成（admission 五类、FundedBackingStorage/ObjectBacking、Tunnel 迁 MemoryObjectCore）；当前任务是**对象侧投影上收**（脱离单页单 PA），完成标准见文档。匿名 backing 重构（旧 B1/C1）已在可行性验证中推翻并降为可延后项 |
...
- 当前：**MemoryPool、MemoryObject 与多页 IPC 数据面**——切片 1–6D 已收口；切片 7 公共 MemoryObject 实施中（见 [`todo-2026-09-memory-object-unification.md`](todo-2026-09-memory-object-unification.md)）：步骤 1–3 已完成（admission 五类 `6e18b8f`、`FundedBackingStorage`/`ObjectBacking` 基座 `6e18b8f`、Tunnel 迁 `MemoryObjectCore` `16dd3b4`）。**当前任务：对象侧投影上收**——`project()` 地基已落地（输出到调用方 Vec），接下来 `prepare_object_mapping` 脱离单页单 PA、删 `object_bytes`。旧 B1/C1 的匿名 backing 重构在实施前可行性验证中被推翻（定长 64 槽非匿名常驻形态、切分不能前移 Commit、owner 进 ledger 需泛型化但公共对象不需要），完整推导在计划文档「已推翻的方案与推导」节；步骤 6–8 待行。原切片 6E–10 计划见 [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md)，方向见 `notes/ideas/{kernel,mm,object,task,bootstrap}.md`。
