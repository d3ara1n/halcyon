...
| [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md) | 原切片 6E–10 计划：切片 1–6D 已完成；6E 部分完成（提交 `2e18c6e`），剩余项并入下条 |
| [`todo-2026-09-memory-object-unification.md`](todo-2026-09-memory-object-unification.md) | **当前主线**：切片 7 公共 MemoryObject。步骤 1–4 已完成（admission 五类、ObjectBacking、Tunnel 迁 MemoryObjectCore、对象映射脱离单页单 PA）；**下一步步骤 6–8**，其中步骤 8（公共 ABI）可独立先行。匿名 backing 重构（旧 B1/C1）已在可行性验证中推翻并降为可延后项 |
...
- 当前：**MemoryPool、MemoryObject 与多页 IPC 数据面**——切片 1–6D 已收口；切片 7 公共 MemoryObject 实施中（见 [`todo-2026-09-memory-object-unification.md`](todo-2026-09-memory-object-unification.md)）：步骤 1–3 已完成（admission 五类 `6e18b8f`、`ObjectBacking` 基座 `6e18b8f`、Tunnel 迁 `MemoryObjectCore` `16dd3b4`）。**步骤 4 已完成**（`51b3742` + `0fad27f`）：`prepare_object_mapping` 接收 view 对象内偏移与投影出的物理 span 序列，逐段 preflight 并经 `publish_batch` 发布，表页按段独立预算；`MapBacking::Object` 的 `object_bytes` 删除，长度真值随 `ObjectViewAuthorization` 从对象状态机流出；单 permit 断言与 `PAGE_SIZE` 硬编码全清，Tunnel 单页退化为长度为一。`ObjectBacking` 改堆化持 extent 列表并删除 `FundedBackingStorage`——定长容器嵌进常驻对象是上一批栈帧抬高的成因，改回后最大帧 0x2390、guard 与审计上限收回 0x3000/0x2800。旧 B1/C1 的匿名 backing 重构在可行性验证中被推翻（定长 64 槽非常驻形态、切分不能前移 Commit、owner 进 ledger 需泛型化但公共对象不需要），完整推导在计划文档「已推翻的方案与推导」节。**下一步步骤 6–8**，步骤 8 可独立先行。原切片 6E–10 计划见 [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md)，方向见 `notes/ideas/{kernel,mm,object,task,bootstrap}.md`。
