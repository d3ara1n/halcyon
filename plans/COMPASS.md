...
- 自然序往后：IPC 契约设计先行拍板（message 邮箱 vs 会合 / signal 投递模型 / tunnel 共享帧所有权，notes/ideas/message.md 现存矛盾需先解决）→ message/tunnel/signal 语义注入 → FAL/FS → 服务化（pm 接管 spawn）→ 设备/中断接入 → 异构（效能核多域、实时核 AMP）。
