...
| [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md) | 当前主线：planner/backing/PTE 已统一覆盖 Building/bootstrap 与异步 Extend，真实 Remote completion/WaitContext/终止 Retire 已闭合；下一步迁移 Tunnel ObjectView/lease 与 HandleClose |
...
- 当前：**用户内存映射**——order 树、affine 帧发布、`MemorySpace` planner、reservation-aware `TableTree`、固定槽 Remote Call 与稳定 AddressSpace identity/epoch 已完成；当前 AddressSpace ledger + `OwnedBacking.extents` 已统一 ProcessMap/Write、ELF、bootstrap stack/StartupBlock 与 Running Extend，旧 `AddressSpace.frames` 和逐页 alloc_map 已删除。Extend 是首个真实异步 MemoryChange consumer：预构造不含 Thread 的 WaitContext 接受 early completion，Commit 登记 lifecycle mandatory operation，最后 ack 后 Retire/Complete，REAPABLE 不早于必成事务收束；virt debug/release 与 race matrix 10/10 通过。下一步迁移 Tunnel ObjectView/lease 与 HandleClose，删除 `external_mappings`/按 VA 解除，再进入公开 MemoryMap/Unmap/Protect 与 rinlib arena/guard。该项收口后重审 ThreadSpawn 的用户栈、结果槽与 join handle 契约。计划见 [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md)。
