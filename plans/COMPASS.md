...
| [`todo-2026-09-memory-object-unification.md`](todo-2026-09-memory-object-unification.md) | **当前主线**：6E 剩余项（ObjectBacking、投影统一、对象侧 admission）与切片 7（公共 MemoryObject ABI/Handle/用户面）合并实施，用最终形态一次设计到位 |
...
- 当前：**MemoryPool、MemoryObject 与多页 IPC 数据面**——切片 1–6D 已收口（平台供给账本、系统储备、MemoryPool、funded broker、Unbound/Bind/root bootstrap、owner/admission、owner-aware 页表协议、分批 deferred retire、root 与全用户页表资金化）；切片 6E 部分完成（提交 `2e18c6e`：匿名 backing 资金化、Tunnel 创建池付费、`alloc_user_*` 生产路径清理），剩余项（`ObjectBacking`、投影统一、对象侧 metadata admission）与切片 7（公共 MemoryObject）**合并为一次统一实施**，用最终形态设计到位，避免为 6E 单独闭合而造临时层。实施计划见 [`todo-2026-09-memory-object-unification.md`](todo-2026-09-memory-object-unification.md)，切片 6E–10 原计划见 [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md)，方向见 `notes/ideas/{kernel,mm,object,task,bootstrap}.md`。
