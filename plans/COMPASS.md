...
| [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md) | 当前主线：切片 1–7 已提交，8A 的公开内存事务/guard/termination 与五条平台矩阵已全绿，待独立提交；随后随 ThreadSpawn 完成同 AddressSpace 多 hart 的 8B gate，全链真实提交后生成统一 user-memory-mapping review 计划 |
...
| [`todo-2026-09-thread-model.md`](todo-2026-09-thread-model.md) | 线程资源模型三批计划：批一及事务复审已收口；批二的用户内存 8A 前置已完成验证，待 8A 提交后重审 UserStack/guard/result/join 组合契约并实施 |
...
- 当前：**用户内存映射两阶段联合收口**——切片 1–7 已提交；order 树、affine 帧发布、`MemorySpace` planner、reservation-aware `TableTree`、Remote Call、统一 AddressSpace、公开 MemoryMap/Unmap/Protect、Tunnel ObjectView lease 与 rinlib `MappedRegion`/多 arena allocator 已贯通，旧 AddressSpace 旁路和 brk/Extend 已删除。8A 新增公开 Map→Protect→Unmap 与异 hart kill、guard fault 进程局部收束，竞态矩阵扩至 12/12；host、debug/release、hetero/nofd/sifive_u 均全绿，sifive_u formal 内核栈按实测组合峰值调整为 `0x9000`，验收高峰前按 lifecycle 状态释放已终止服务 AddressSpace。8A 当前待独立提交；之后重审并实施 ThreadSpawn，以真实同一 AddressSpace 多 hart、并发解除和次线程资源所有权完成 8B。计划见 [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md)。
