...
- **资源能力与 Job 正交**：Job 只做创建、成员与收束；内存由可派生 MemoryPool 页额度支付，CPU 由预约对象支付，设备由各自 capability 授权。capability 跨 Job 转移不改资源来源（`notes/ideas/{task,mm,object}.md`）。
...
| [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md) | 当前主线：MemoryPool 与进程绑定前置 → 公共 MemoryObject/多页 ObjectView → 多页 Tunnel 与 Runnel v2 |
...
- 当前：**MemoryPool、MemoryObject 与多页 IPC 数据面**——IPC 数据面设计已确认 MemoryObject-first：显式页额度 MemoryPool 与全局物理库存分离，Job 删除预算职责；公共 MemoryObject 复用多 extent backing，Tunnel 扩为有界多页，Runnel v2 保持字节流，BufferQueue 作为独立用户态协议。实施计划见 [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md)，设计档案见 [`archived/todo-2026-09-ipc-data-plane-design.md`](archived/todo-2026-09-ipc-data-plane-design.md)。
