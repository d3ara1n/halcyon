...
| [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md) | 当前主线：平台供给、系统储备、MemoryPool、funded frame、空壳 ProcessBindMemory、root bootstrap、资金化 owner/admission、owner-aware 页表协议与分批 deferred retire 已完成；切片 6D 已完成 root、Running/Tunnel 与 Building ProcessMap 的主体 funded transaction，仍需删除剩余 transitional raw 页表路径并收口错误分类与专门失败测试；随后进入匿名 backing 全面资金化，再进入公共 MemoryObject/多页 Tunnel/Runnel v2 |
...
- 当前：**MemoryPool、MemoryObject 与多页 IPC 数据面**——平台供给账本（`198e665`）、物理隔离系统储备（`0a944c7`）、strict count-solvent MemoryPool、funded frame broker、切片 4+5 的 Unbound Process/一次性 ProcessBindMemory/root Pool bootstrap、切片 6A/6B 的资金化 owner/admission 与 owner-aware 页表生命周期协议，以及切片 6C 的分批 deferred retire（`7225673`）已经收口；切片 6D 已完成 root owner、Running/Tunnel 和 Building `ProcessMap` 的主体跨锁 funded transaction，并将 Building `image_end` 纳入提交语义。当前剩余自然序是删除 Building/ELF/bootstrap 的 transitional raw 页表 API、恢复 funded error 分类并补齐失败/基线测试，随后进入匿名 backing 全面资金化。实施计划见 [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md)，方向见 `notes/ideas/{kernel,mm,object,task,bootstrap}.md`。
