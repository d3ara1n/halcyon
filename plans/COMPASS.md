...
| [`todo-2026-09-ipc-data-plane-design.md`](todo-2026-09-ipc-data-plane-design.md) | 当前主线：以已收口的 MemoryObject、ThreadSpawn 与 IPC 压力证据设计公共对象 ABI、帧移交、Tunnel/Runnel 与描述符协议 |
...
- 当前：**IPC 数据面设计**——用户内存切片 1–8B 与 ThreadSpawn 三批已全部收口：`bdc83ef` 完成固定宽 ABI、Running `Spawning→Ready`、ThreadControl DONE、线程级 Map-result obligation 与 guarded `UserStack`/结构化 `JoinHandle`，`004cae5` 完成 16/16 竞态矩阵、carryover IPC 压力线和 FramePool 部分 extent 归还修复；common debug/release、hetero、nofd、host 148/148 与 `sifive_u` 连续十轮全绿。实施档案见 [`archived/todo-2026-09-user-memory-mapping.md`](archived/todo-2026-09-user-memory-mapping.md) 和 [`archived/todo-2026-09-thread-model.md`](archived/todo-2026-09-thread-model.md)，当前设计计划见 [`todo-2026-09-ipc-data-plane-design.md`](todo-2026-09-ipc-data-plane-design.md)。
