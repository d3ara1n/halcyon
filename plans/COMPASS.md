...
| [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md) | 当前主线：order 树、affine 帧发布与纯逻辑 MemorySpace/MemoryChange planner 已完成；下一步实施页表资源预留，再进入 Remote Call 与统一 AddressSpace |
...
- 当前：**用户内存映射**——统一所有权、Map 提交前结果承诺、AllocationKey/RegionKey 精确切割、shootdown happens-before 与 MemoryObject sealing 方向已确认；order 树、affine 帧发布 seam 和 host 可测的 `os/memory_space` planner 已完成。planner 已覆盖固定容量 ledger、精确 Map/Unmap/Protect、类型化 MemoryChange、UserWriteLease 与 WritePermit/seal，但尚未接入内核 AddressSpace。下一步按页表资源预留 → Remote Call → AddressSpace 调用者迁移 → Running ABI/rinlib 的顺序实施。`AddressSpace.frames`、`external_mappings`、brk/Extend 与旁路 shootdown 最终删除；该项收口后重审 ThreadSpawn 的用户栈、结果槽与 join handle 契约。计划见 [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md)。
