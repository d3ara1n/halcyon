...
| [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md) | 当前主线：order 树、affine 帧发布、MemorySpace planner、reservation-aware TableTree 与 Remote Call/AddressSpace epoch 基础已完成；下一步统一 ledger/backing transaction 并把 completion 闭合到 WaitContext/终止/Retire |
...
- 当前：**用户内存映射**——统一所有权、Map 提交前结果承诺、AllocationKey/RegionKey 精确切割、shootdown happens-before 与 MemoryObject sealing 方向已确认；order 树、affine 帧发布 seam、`os/memory_space` planner、reservation-aware `os/page_table`、固定槽 `os/remote_call` 与稳定 AddressSpace identity/epoch/execution gate 已完成。virt debug/release 和 sifive_u 均通过运输及 active snapshot shootdown 自检和 10/10 竞态矩阵。下一步把 planner、真实 backing/PTE 与 completion 组成统一 AddressSpace transaction，闭合 WaitContext、终止接管和 ack 后 Retire，再迁移 Building/bootstrap/Tunnel 调用者；`AddressSpace.frames`、`external_mappings`、brk/Extend 与旁路 shootdown 最终删除。该项收口后重审 ThreadSpawn 的用户栈、结果槽与 join handle 契约。计划见 [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md)。
