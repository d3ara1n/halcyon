...
| [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md) | 当前主线：切片 1–7 与 8A 已提交；ThreadSpawn 批二已完成同 AddressSpace 多 hart、并发解除、guarded stack/join 的 8B gate，待独立提交；全链真实提交后生成统一 user-memory-mapping review 计划 |
...
| [`todo-2026-09-thread-model.md`](todo-2026-09-thread-model.md) | 线程资源模型三批计划：批一已收口；批二 ThreadSpawn/Exit/Yield、ThreadControl DONE、guarded UserStack/JoinHandle 与 user-memory 8B 已实现并全绿，待独立提交；批三竞态扩面、carryover IPC 压力线与实现文档同步待做 |
...
- 当前：**用户内存 8B 与 ThreadSpawn 批二待提交**——用户内存切片 1–7 及 8A 已提交；shared 固定宽 ThreadSpawn ABI、Running `Spawning→Ready` 事务、ThreadControl DONE、线程级 Map-result obligation、rinlib guarded `UserStack`/结构化 `JoinHandle` 已贯通。真实负载覆盖同 AddressSpace 旧 VA 精确复用与双 hart shootdown、8 轮普通 Unmap/Tunnel lease close 并发、join/stack 回收、末线程 `Exited(0x8b)`；common debug/release、virt-hetero、virt-nofd 与 `sifive_u` 均为 13/13，host 逻辑矩阵全绿。短时序 kill 另命中并修正 ThreadSpawn draining 回滚的 `ADDRESS_SPACE → LEAF` 锁序反转。计划见 [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md) 与 [`todo-2026-09-thread-model.md`](todo-2026-09-thread-model.md)。
