...
| [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md) | 当前主线：平台供给账本与系统储备已完成；下一步为 strict MemoryPool 状态机 → funded frame broker → 空壳 ProcessBindMemory → 公共 MemoryObject/多页 Tunnel/Runnel v2 |
...
- 当前：**MemoryPool、MemoryObject 与多页 IPC 数据面**——平台供给账本与“用户 FramePool + typed system tickets”的物理隔离系统储备已经收口；下一自然序是 strict count-solvent MemoryPool 状态机，随后依次建立 funded frame broker、Building 空壳的一次性 ProcessBindMemory 与 bootstrap 同构，其上公共 MemoryObject 复用多 extent backing，Tunnel 扩为有界多页，Runnel v2 保持字节流。实施计划见 [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md)，方向见 `notes/ideas/{mm,object,task,bootstrap}.md`。
