...
| [`todo-2026-09-thread-model.md`](todo-2026-09-thread-model.md) | 线程资源模型三批计划：批一及事务复审已收口；批二 ThreadSpawn/Exit/Yield + join 壳待做 |
| [`todo-2026-08-29-early-quiescent-shutdown.md`](todo-2026-08-29-early-quiescent-shutdown.md) | 负载存活时误判 quiescent 停机（virt 234158 vs 正常 248843 帧）；在 sifive_u 上与挂死同形，但是否与旧现场同源及引入批次均未定性 |
...
- 当前：**用户态多线程（ThreadSpawn）里程碑**——前置全部就绪（teardown barrier、线程成员表、active 位图、每线程 FP 状态与域绑定）。批一已把普通组装拆为 ProcessGrant/ProcessAttach/ProcessStart：Grant 与 Attach 是 Building 期独立事务，Start 只冻结 execution binding、消费 builder 并原子发布完整 Ready 批次；普通 StartupBlock 由用户态构造，init bootstrap 仅保留内核内嵌同构序列。首轮与事务复审档案见 [`archived/review-2026-08-29-thread-model-batch1.md`](archived/review-2026-08-29-thread-model-batch1.md) 和 [`archived/review-2026-08-29-thread-model-transaction-convergence.md`](archived/review-2026-08-29-thread-model-transaction-convergence.md)。下一步是批二 ThreadSpawn/Exit/Yield + join 壳。
