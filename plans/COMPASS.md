...
| [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md) | 当前主线：平台供给、系统储备、MemoryPool、funded frame、空壳 ProcessBindMemory、root bootstrap、资金化 owner/admission、owner-aware 页表协议与分批 deferred retire 已完成；切片 6D 已完成 root、Running/Tunnel、Building ProcessMap、ELF、bootstrap 与 init stack 的 funded transaction，并删除 transitional raw 页表路径、恢复错误分类与失败基线自检；随后进入匿名 backing 全面资金化，再进入公共 MemoryObject/多页 Tunnel/Runnel v2 |
...
- 当前：**MemoryPool、MemoryObject 与多页 IPC 数据面**——平台供给账本（`198e665`）、物理隔离系统储备（`0a944c7`）、strict count-solvent MemoryPool、funded frame broker、切片 4+5 的 Unbound Process/一次性 ProcessBindMemory/root Pool bootstrap、切片 6A/6B 的资金化 owner/admission 与 owner-aware 页表生命周期协议，以及切片 6C 的分批 deferred retire（`7225673`）已经收口；切片 6D 已完成 root owner、Running/Tunnel、Building `ProcessMap`、ELF、bootstrap 与 init stack 的跨锁 funded table transaction，并删除 transitional raw 页表路径、恢复 funded error 分类及失败基线自检。当前剩余自然序是匿名 backing 全面资金化（切片 6E），随后进入公共 MemoryObject、多页 Tunnel 与 Runnel v2。实施计划见 [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md)，方向见 `notes/ideas/{kernel,mm,object,task,bootstrap}.md`。
