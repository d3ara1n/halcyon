...
#14 公共时间已完成并归档：CSR/内存采样排序、可读/可编程期限尾部、高水位/失败隔离、Wait/Sleep/Send 到期与 ownership、运行期协作停止及真实消费者验证均已通过；实现见 `notes/impls/time.md`。下一任务是 #15，researcher/explorer 继续使用有额度 provider 的 DeepSeek Flash。
...
| [`todo-2026-09-fal-service-capabilities.md`](todo-2026-09-fal-service-capabilities.md) | FAL 业务暂停、总体未交付：公共对象 #13 与时间 #14 已完成，先完成运输/RPC/执行 #15，随后恢复授权后端与业务 |
| [`todo-2026-09-monotonic-time-rpc-deadline.md`](archived/todo-2026-09-monotonic-time-rpc-deadline.md) | 时间前置 #14 已完成并归档：精确时钟、MonotonicNow、绝对 Wait/Sleep/Send、运行期协作停止与现有消费者；由执行/业务任务消费完整 Deadline |
...
公共对象前置 #13 已完成并[归档](archived/todo-2026-09-13-public-ipc-wait-prerequisites.md)，实现与验证真值见 `notes/impls/ipc.md`：Native 坏输出/新轮停驻旧取消、通知历史/终态摘槽/非空压力、真实双接收线程与 forced Full、跨进程提交后 kill 和 GDB 已装 Close/active 窗口已补，旧 findings 复核关闭；core/128MiB/release/nofd/启动失败、七面 lint、163 项 host 通过。完整 stress 的旧截断/概率判定仍由 #17 独立延期，未宣称通过；纳入当前 FAL 分支集成基线，提交定位见总计划交接节。下一自然序 #15 → FAL，workspace 整理继续延期。
...
- 计划先审视再实施：notes/plans 是可修正参考，不是冻结真值；开工反推全部前置，按机制闭合成任务，自底向上完成。强耦合 ABI/通知/所有权/退休共同迁移，不按文件或 happy path 拆散；完整前置必须验证，分片通过不等于整体交付。
