...
| [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md) | 当前主线：平台供给账本/系统储备 → strict MemoryPool 状态机 → funded frame broker → 空壳 ProcessBindMemory → 公共 MemoryObject/多页 Tunnel/Runnel v2 |
...
- 当前：**MemoryPool、MemoryObject 与多页 IPC 数据面**——先闭合平台供给和物理隔离的系统储备，再实现 strict count-solvent MemoryPool 与 funded frame broker，继而建立 Building 空壳的一次性 ProcessBindMemory 与 bootstrap 同构；其上公共 MemoryObject 复用多 extent backing，Tunnel 扩为有界多页，Runnel v2 保持字节流，BufferQueue 作为独立用户态协议。实施计划见 [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md)，方向见 `notes/ideas/{mm,object,task,bootstrap}.md`。
