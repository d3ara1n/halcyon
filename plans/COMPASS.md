...
| [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md) | 当前主线：切片 1–7 已完成，AddressSpace 与公开 MemoryMap/Unmap/Protect、rinlib affine region/allocator arena 已贯通，Extend 退役；下一步切片 8 统一完成 host/多 hart/平台矩阵与文档收口，全链真实提交后生成统一 user-memory-mapping review 计划 |
| [`todo-2026-08-30-remote-call-review.md`](todo-2026-08-30-remote-call-review.md) | 未来审查：提交 `6199985` 的固定槽 token、RVWMO/SBI 边、active/epoch gate、全量 fence 与后续 MemoryChange/终止/Retire 组合 |
...
- 当前：**用户内存映射**——切片 1–7 已完成。order 树、affine 帧发布、`MemorySpace` planner、reservation-aware `TableTree`、Remote Call 和统一 AddressSpace 已覆盖 ProcessMap/Write、ELF/bootstrap、公开 MemoryMap/Unmap/Protect 及 Tunnel ObjectView lease；旧 `AddressSpace.frames`、`external_mappings`、逐页 alloc_map、brk/Extend、按 VA 外部解除和本地-only shootdown 已删除。公开 Map 使用固定宽 result + release cookie，rinlib 以 affine `MappedRegion` 和固定容量多 arena allocator 持有解除责任；debug/release QEMU 已覆盖双 guard、Protect、部分 Unmap、同 VA 不同 backing 重映射和清零，release 同步闭合 `SumGuard` inline asm 的 compiler memory clobber。下一步是切片 8 的多 hart/hetero/nofd/sifive_u 竞态矩阵与最终文档收口；全链真实提交后按主计划生成统一 user-memory-mapping review 计划，再重审 ThreadSpawn 用户栈、结果槽与 join handle 契约。计划见 [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md)。
