...
| [`todo-2026-09-memory-object-unification.md`](todo-2026-09-memory-object-unification.md) | **当前主线**：6E 剩余项与切片 7 合并。步骤 1–3 已完成（admission 五类、FundedBackingStorage/ObjectBacking、Tunnel 迁 MemoryObjectCore）；下一步从 B1/C1 分叉点（投影上收深度）重启，文档内已列三选项与推荐 |
...
- 当前：**MemoryPool、MemoryObject 与多页 IPC 数据面**——切片 1–6D 已收口；6E 剩余项与切片 7 合并实施中（见 [`todo-2026-09-memory-object-unification.md`](todo-2026-09-memory-object-unification.md)）：步骤 1–3 已完成——对象侧 metadata admission 五类（`6e18b8f`）、`FundedBackingStorage`/`ObjectBacking` 基座（`6e18b8f`）、Tunnel `Connection` 迁至统一 `MemoryObjectCore` 并删除私有 ObjectId 铸造（`16dd3b4`）；栈窗口随固定容量 backing 的实测帧上调（guard 0x3000→0x4000、审计上限 0x2800→0x3800、sifive_u formal 栈→0xF000）。**下一步从 B1/C1 分叉点（投影上收深度）重启**，文档内已列 A/B/C 三选项与推荐；步骤 6–8 待行。原切片 6E–10 计划见 [`todo-2026-09-memory-object-data-plane.md`](todo-2026-09-memory-object-data-plane.md)，方向见 `notes/ideas/{kernel,mm,object,task,bootstrap}.md`。
