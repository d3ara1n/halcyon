...
| [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md) | 当前主线：切片 1–6 已完成，所有现有用户映射调用者已统一到 AddressSpace；下一步公开 MemoryMap/Unmap/Protect、迁移 rinlib arena/guard 并退役 Extend；全链真实提交后生成统一 user-memory-mapping review 计划 |
...
- 当前：**用户内存映射**——切片 1–6 已完成。order 树、affine 帧发布、`MemorySpace` planner、reservation-aware `TableTree`、Remote Call 和统一 AddressSpace 已覆盖 ProcessMap/Write、ELF/bootstrap、Running Extend 及 Tunnel ObjectView lease；旧 `AddressSpace.frames`、`external_mappings`、逐页 alloc_map、按 VA 外部解除和本地-only shootdown 已删除。TunnelCreate/Attach/Endpoint HandleClose 以 WritePermit + prepared WaitContext + mandatory Remote completion 闭合，ProcessDrain 可接管 detached close 并在 Busy 时保存 entry 重试；debug/release QEMU 的 Tunnel 64 轮、最小预算 drain 与 race matrix 10/10 通过。下一步是切片 7：公开 MemoryMap/Unmap/Protect、rinlib affine region/allocator arena/guard，并退役 brk/Extend；全链真实提交后按主计划生成统一 user-memory-mapping review 计划，再把主线移到 ThreadSpawn 用户栈、结果槽与 join handle 契约重审。计划见 [`todo-2026-09-user-memory-mapping.md`](todo-2026-09-user-memory-mapping.md)。
