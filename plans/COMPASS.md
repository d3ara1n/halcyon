...
- 当前：**完整进程生命周期 step 2–4 已落地并已过统一代码 Review**——ProcessControl 前移到 Building（32B CreateResult/control_rights 显式请求）、lifecycle 顶级锁状态机（Building→Running→Terminating→Dead；ThreadRecord 含 Staging）、全局进程表退役（Job 成员表为唯一强持根 + S1 所有权图 + static anchor）、跨 hart kill 最小正确版（IPI + trap 入口统一吸收 + 出口 satp/SFENCE 先行）、**硬上界 ProcessDrain**（FreeScan 游标/HandleTable Pinned/leak_root/pending-free 全链真实有界）、init 五服务监督闭环（srv_target live kill 验收 Dead/Killed）与 libprocess 失败路径自收束。验收面：host 75 单测/virt ×6/sifive_u。Review 七轴全过无 P0/P1（PLR-1 契约表述已修正，PLR-2 登记；报告见 [archived/review-2026-08-27-process-lifecycle-code-review.md](archived/review-2026-08-27-process-lifecycle-code-review.md)）。FAL 剩余面（DirectoryGrant、每订阅者 watch、跨进程 provider）仍排在其后。
