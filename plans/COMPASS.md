...
- StartupBlock v2 已落地：outer 为内核生成 Header + 实际 child Handle 数组 + opaque payload，不再使用 descriptor/tag 或固定 slot/generation；boot loader 暂以 Handle[0] 约定 init/pm mailbox。实现现状见 `notes/impls/startup.md`。
