...
7. 文档是否明确跨硬件 epoch、RPC/FAL、#17 的边界；是否保留完整日志且不把主动收割或基础设施竞争写成 guest 成功。

## 验证

本提交已有 shared 27 项、os 140 项 host、`just check`、七面 clippy、virt core/release/nofd、sifive_u core 和 runtime-stop GDB probe（PARKED `0xf`）证据。日志位于本机 `artifacts/check/time-*`，不随 Git 提交；异机必须按当前源码重跑。完整 stress 的概率失败与 Tunnel 静默截断仍归 #17。
