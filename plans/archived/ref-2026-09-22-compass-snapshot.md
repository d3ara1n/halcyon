...
> 2026-09-22，分支 `task/fal-service-capabilities`，HEAD `2811d48`。保存整理前工作树中的完整导航（含未提交 FAL 接力更新），只作历史追溯，不安排当前任务。原文中的“当前”“下一步”和规则均属于当时记录；现行入口为 [COMPASS](../COMPASS.md)，施工规则为 [AGENTS](../../AGENTS.md)。
> 正文保留原文，Markdown 相对链接统一增加一级目录以保持原目标；反引号中的路径仍按原文上下文解释。
...
**当前任务**：[FAL 后端、授权与业务](../todo-2026-09-fal-service-capabilities.md) 的 F3d 结构回改中，分支 `task/fal-service-capabilities`、基线 `2811d48`，未提交工作树保持。A 单 FAL Mailbox/GrantTable 持内存/发现双根，注册控制另有入口。D1 已删 `World.route` 全局前缀捷径：唯一绑定由 ServiceBackend 附着于内存根，后端 Lookup 核验根/组件/权限后复制目标 owner；真实 Registry 根查 `second` 为 NotFound，合法 `/second` 仍走 B。Property commit 复核 Take 预留。D2/D3 局部已用 Registry Starting 索引条件到期代替 `establish_active`，Read/Delegate 的提交与取消共用单一意图槽、Pending 经 Dispatcher.cancel 等回调；host 34/16 项、七面 lint 与 core QEMU 仅是开发证据。下一步从普通/注册 Ingress 的入箱→Gate→Outbox 来源准入→业务提交链收束正常满载与 owner，随后独立验证上游放弃而 A 继续运行；D4 provider 提取不得越过这些阻断。唯一计划 §8.8 与末尾接力拥有自然顺序，历史 core/host 不代表完整交付。
...
施工自然序为 F3d 注册/发现与投影 → F3e Open → F3f 流 Copy → F4 独立 `test_fal` 与组合验收；技术上 F3e 不依赖 F3d，F3f 依赖 F3e，F4 汇合全部责任。F3e/F3f 的公共 owner/付款/退出边界已定，详细控制状态机、wire 和结果 API 仍须各自在开工前完成设计。已有公共机制、库重排和 F0–F3c 证据保留，整体 FAL 尚未交付。
...
| [`todo-2026-09-fal-service-capabilities.md`](../todo-2026-09-fal-service-capabilities.md) | 唯一 FAL 任务：F3d 正常拓扑已有开发检查；现按 §8.8 的 D0–D5 回改 F3a/F3c 全局路由、Registry 状态、请求准入/取消、Backend/provider 接口与失败/退款矩阵。末尾记录确定缺陷和历史证据；第 9/10 节仍保留 Open/Copy 局部设计门，第 12 节汇合组合门 |
