...
- 已验证：四服务（fs/init/pm/drv_spi_sifive）在 virt 4 核、virt 1 核、sifive_u 5 核下全部完成用户态 main；fs 因 FAL 未实现「干净被杀」；pm 的 sleep 异步通路真实睡眠唤醒。旧内核单核挂起、SMP 别名 UB、进程不回收三大死穴均已在增量证伪中。virt 30+ 轮压测全过；sifive_u 卡死双根因已修（slot/raw 混用、Extend 错配 OOM 自锁），静默多数轮次收敛，仅余非确定性内核野跳转（cause=0xc pc=0x0，证据已取得）待归因。
