...
- **施工服从整体交付**：总体方案冻结后可按依赖自下向上施工，阶段代码允许暂未接通后续调用者；每个分片必须直接形成最终结构的一部分，并在总计划登记其连接点、剩余责任与整体完成门。局部编译、测试或提交只证明施工点可回溯，不构成专题完成；只有全部真实调用者、失败/退役路径、旧机制删除、组合验证与原 Review 复核同时满足，才标记整体交付。
...
- **阶段收口包含残留审计**：测试通过不等于阶段完成。每个核心阶段结束前必须全仓搜索旧 API、旁路资源来源、重复 owner/permit 真值、单用途 adapter、过渡注释和文档错位，并将结果归类为“长期保留、统一重构、删除、另案立项”。
