...
- **最终形态优先**：核心机制开始编码前，先冻结最终类型图、所有权图、锁阶与失败边界；阶段性实现只能缩小已闭合的能力范围，不得用临时抽象替代尚未决定的最终结构。
- **施工服从整体交付**：总体方案冻结后可按依赖自下向上施工，阶段代码允许暂未接通后续调用者；每个分片必须直接形成最终结构的一部分，并在总计划登记其连接点、剩余责任与整体完成门。局部编译、测试或提交只证明施工点可回溯，不构成专题完成；只有全部真实调用者、失败/退役路径、旧机制删除、组合验证与原 Review 复核同时满足，才标记整体交付。禁止在实施期间进行局部验证或验收，以此避免为了验收而把任务拆的细致，并编写最小实现或补丁代码来通过验收，导致过渡或补丁代码永久残留损坏设计。遇到需要降级实现的地方，应该回过头来看看是否是机制本身的问题，如果机制无法闭环就需要重构机制；如果是机制正确但是无法局部闭环，那就等其他部分完成一起闭环。永远不进行局部闭环和验收。
