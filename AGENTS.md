...
- **notes/ 按视角分层，不是按阶段**：`ideas/` 与 `impls/` 是看待同一系统的两个视角，不是同一篇文档的前后状态。
    ...
    - **impls/ 写「当前实际怎么做」**：按组件与机制组织的当前实现说明，正文解释模块关系、数据结构、数据与控制流、所有权、生命周期、并发和失败处理，引用具体源码路径供定位与核实。读者无需了解任务背景、阅读计划或翻查源码，即可理解本主题的关键实现。施工期间同样描述当前面貌，包括部分接通的机制、尚未实现的能力及其行为限制；随代码演进改写对应章节、清除过时叙述，使全文保持一致的当前视图。
    - **plans/ 承载任务过程**：任务编号、阶段划分、基线提交、施工与决策过程、检查和验收结果、阻塞项、剩余工作及接力顺序，均由对应计划或审查档案拥有。impls 可链接这些记录供追溯，正文的组织与解释独立于任务上下文；测试机制及其覆盖边界可进入实现说明，某轮运行结果与完成宣告留在计划。
...
- 设计取舍的现行结论与理由按视角写入 notes/；选型经过与迁移记录留在对应计划，代码历史由 git log 追溯。代码与实现说明描述当前结构。
...
- **`todo-<日期>-<主题>.md`**：专题计划与工作记录，拥有目标、实施安排、进度和验证证据。完成后归档；需长期维护的知识按所属视角整理进 notes/，仍存问题进入 KNOWN_ISSUES 或对应后续计划，过程记录留在档案。子类【未来审查计划】（`todo-<日期>-<主题>-review.md`）在**提交之后**生成：记录任务对应的提交哈希与改动概要，供日后 Review 对照（Review 的对象是提交）；
- **`<类型>-<日期>-<主题>.md`**：调查复盘与参考资料（现有类型 `review-` 调查归档、`ref-` 对照资料），只读；
- **`archived/`**：已结束的计划与过程档案，保留历史证据供追溯，只进不出；当前实施安排由活跃计划拥有；
...
4. **设计闭包**：确定目标类型图、所有权/授权图、状态机、线性化点、锁阶、停驻/唤醒、失败/取消/退出/退款及公开语义；涉及硬件、ABI 或协议时先取证。完成标准是当前范围语义闭合，未决选择均已裁决或成为阻塞项，方向结论进入 `notes/ideas/`，任务基线盘点与待实施方案进入唯一计划；`notes/impls/` 按「心智模型」维护当前实现说明。
...
7. **结构收口 Review**：对照最终设计检查 bug、性能、锁序、owner、重复真值、旧路径、文档一致性和残留清单；已有 findings 必须复核。完成标准是开放问题均有关闭证据或唯一延期条目，相关 impls 与当前代码一致，且无需任务背景即可理解其关键实现。
8. **组合收口与归档**：完成跨机制组合门，确认 `notes/impls/` 已反映当前实现，将完成状态与验证证据写回唯一计划，并更新 `plans/COMPASS.md` 的导航；完成的计划归档，提交前展示摘要并取得授权，提交后登记固定提交 Review。实现证据推翻设计时，停止当前步骤并回到规模审计/设计，不以编译通过掩盖边界错误。

### 跨会话接力

专题不要求在单一会话内完成；任务按唯一计划记录的自然顺序跨会话接力。若当前会话无法完成整个语义闭包，结束前必须把已完成的施工阶段、未完成责任链、当前代码/文档真值、阻塞项、下一步顺序和验证状态写回唯一 `plans/todo-*.md`，并同步 `plans/COMPASS.md` 的导航；实现变化同步改写 `notes/impls/` 中对应机制的当前说明，方向或契约变化同步 `notes/ideas/`，各自产物遵循「心智模型」的文档职责。后续会话先按接手与基线流程读取这些记录，再从唯一未完成位置继续，不把局部开发检查误写成验收证据，也不重新拆出平行任务或重复已完成工作。只有完整闭包收口后，才登记最终验收、Review 和归档。
