...
6. **分层验证**：施工阶段只做开发检查——编译、类型检查、受影响的 host/target 检查，以及稳定纯逻辑、状态推进和所有权转换的 testcase；这些结果用于发现实现错误，不构成临时阶段的独立验收。待当前语义闭包的真实消费者、正常/失败/取消/退出/超时/退款路径、旧路径删除和最终 owner 责任全部接通后，再统一执行 `just check`、`just clippy` 及必要 QEMU/平台/退出组合。不得为未闭合机制制造测试专用服务、临时消费者或独立验收意义；失败保留完整日志和责任定位，局部通过不得冒充专题或机制完成。
...
专题不要求在单一会话内完成；任务按唯一计划记录的自然顺序跨会话接力。若当前会话无法完成整个语义闭包，结束前必须把已完成的施工阶段、未完成责任链、当前代码/文档真值、阻塞项、下一步顺序和验证状态写回唯一 `plans/todo-*.md`，并同步 `plans/COMPASS.md`；实现事实按需要同步 `notes/impls/`，方向或契约变化同步 `notes/ideas/`。后续会话先按接手与基线流程读取这些记录，再从唯一未完成位置继续，不把局部开发检查误写成验收证据，也不重新拆出平行任务或重复已完成工作。只有完整闭包收口后，才登记最终验收、Review 和归档。
...
- **设计债务必须登记**：任何为集中施工而暂时保留的旧结构、重复表示、兼容接缝、未决抽象或验证缺口，都必须在唯一专题计划中记录现状、目标、owner、触发条件、删除条件和验证标准；若属于长期施工纪律，再在本文件保留规则。未登记的设计债务不得进入下一阶段，不能以编译通过、局部测试通过或阶段提交掩盖。
- **阶段不等于完成**：机制闭包可以按内聚阶段施工以集中注意力，但阶段只限制施工范围，不形成独立交付或整体验收结论。只有全部真实消费者、失败/取消/退出/退款路径、旧路径删除和最终组合验证闭合后，才能更新计划为完成并归档。
...
- **铺路面与消费者**：无即时消费者不是删除或禁建的理由——系统完整性与为后续闭包铺路都是正当交付。无消费者的能力可以作为铺路实现保留，但必须在唯一专题计划中登记其用途、目标消费者、接通条件，以及临时代码/过渡代码的保留期限、删除条件和清理验证；计划内明确的未来消费者同样构成责任落点。已铺路的实现默认续用，替换须有严格更好的替代（正确性、解耦或语义消重）并记录依据；删除判据是「有更好替代」或「确证错误」，不是「暂无消费者」。闭包声称完成时，当前承诺范围内的调用者、失败/清理路径必须闭合；若消费者尚未实施，应如实标记为铺路或待接通，不得把源码存在误写成能力已交付。形状依赖首个真实消费者使用模式的机制先定接缝，不提前建无法确定的具体实现。
