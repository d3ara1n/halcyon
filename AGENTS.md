...
- **决策边界**：默认直接采用经过论证的推荐方案推进。只有会改变已确认外部语义、无法形成语义闭包，或 notes/plans 与目标冲突的架构点才暂停，列出选项与理由交用户拍板；普通内部结构、容量布局和实现取舍由实施者自行决定。
- **决策即文档**：讨论的结果和决策可以得出的宽泛的设计需要进入到 notes 文档以便固化成长久的设计和方向，而非实际的代码与实施细节。
- **计划先审视再实施**：每次开始或恢复专题，实施者先核对目标、范围、外部证据、当前代码、前置依赖、所有权/锁序、失败/取消/退出/退款及验证门，再判断计划是否完整、粒度是否合理。notes 和 plans 是可修正的设计参考与记录，不是因“已确认”“已冻结”便不可质疑的稳定真值；规范性外部契约仍以固定规范取证。计划审视是实施者的开工职责，不等同于交给代码 reviewer 的正式 Review，也不因先前讨论或已有草稿而省略。
- **前置成任务，耦合不拆散**：从目标自顶向下追踪全部前置，形成依赖图，再按依赖自底向上实施。完整公共前置应先独立立案并逐项完成，后续业务任务显式依赖它们；强耦合的 ABI 两侧、所有权转移、来源通知与退休等必须作为同一闭合任务共同设计和迁移，不按文件、syscall 或 happy path 机械拆分。任务粒度以能独立论证的机制闭包为准，既不把未做前置混入巨型业务任务，也不靠临时 adapter 把互相依赖的机制硬拆开。
- **独立理由**：方案可以与旧实现或任何参照系殊途同归，但理由必须从需求独立推导成立；「沿用」「以前就这样」不是理由。
- **工程限额必须有依据**：栈容量、guard 布局、审计阈值及其它自设容量不是架构公理。合理实现因旧值无法通过审计或验收时，先核实硬件/ABI/正确性约束与资源成本；没有独立依据的限额应放宽并同步重校布局、审计和文档，不为通过旧门槛反复拆帧、压缩通用代码或引入 adapter，也不迁就验收平台的任意小容量。必须保留真正的安全不变量，阈值能从布局或规范推导时只保留一个真值来源。
- **高起点**：设计以「成熟系统会怎么做」为目标（生产系统为主要参照，引用前先查证官方文档/源码，不凭印象），不以兼容、延续、仿照现有代码为目标。
- **外部契约先取证**：凡语义由硬件、ABI 或协议决定，编码和 review 前先从 `references/CONTRACTS.md` 定位固定规范并引用具体章节；外部实现只作参照，`pre-ai` 只作缺陷样本，记忆不作证据。
- **广泛参照**：需要外部调研时，从 `references/systems/INDEX.md` 广泛取样，不默认聚焦最知名或最成熟的系统，也关注新兴系统、相邻范式与失败经验；参照用于拓宽选项，设计结论仍从项目需求独立推导。
- **长期合理性优先**：设计与实现不以最短路径、最小改动或只通过当前验收为目标，应从整体所有权、生命周期、失败边界和未来扩展选择机制。阶段实现可以从简，但结构应允许后续以能力增量或实现替换演进。
...
- **延后即立案**：完整能力缺失只有在当前缩小范围仍语义闭合且补齐 ROI 较低时才能暂缓；同一缺口只进入一个 `todo-*.md` 真值点，写明前置/触发条件、完成标准与自然实施顺序，并在影响主线导航时同步 `plans/COMPASS.md`。聊天中的口头延期不构成备案。
...
- **最终形态优先**：核心机制开始编码前，先明确目标类型图、所有权图、锁阶与失败边界及其依据；实施证据推翻前提时先修订设计和依赖图，再继续施工。“明确目标结构”不意味着冻结文档或禁止重新论证；阶段性实现只能缩小已闭合的能力范围，不得用临时抽象替代尚未决定的最终结构。
- **施工服从整体交付**：计划审视完成后按依赖自下向上施工；完整前置任务包含其当前真实调用者、失败/退役路径、旧机制清理与适当验证，完成后再启动依赖它的业务任务。强耦合机制尚未共同接通时，分片代码允许暂未闭合，但不得宣称任务完成或用兼容/补丁路径制造局部闭包。编译、单测、静态检查和完整前置的组合验证用于尽早检验正确性，不以禁用验证维持整体性；禁止为局部过关降低目标、机械拆碎任务或留下过渡机制。总体计划持续登记前置完成证据、后续连接点、剩余责任与整体完成门；只有全部真实调用者、失败/退役路径、旧机制删除、组合验证与已有代码 Review findings 复核同时满足，才标记整体交付。
- **临时机制必须可删除**：任何过渡类型、字段、adapter、兼容分支或重复验证路径，必须在同一计划中登记替代物、保留期限、删除触发条件和清理验证；无法写出删除条件的机制不得引入主线。
- **残留即见即清**：清理不集中在阶段末。实施途中撞见的过渡类型、单用途 adapter、重复真值或错位命名，只要能在当前改动的语义闭包内消灭就顺手消灭，不求成套完整——清一处算一处；清不掉的立即登记，不留口头账。顺手清理以当前改动的正确性论证仍自洽为界，超出即转登记。
- **结构收口 review**：review 不仅检查 bug、性能和测试，还必须对照最终设计识别 A→B 迁移中遗留的 C/D/E/F；确认 B 后直接迁移调用点并删除旧路径，不以兼容层或继续叠加条件分支掩盖未完成的重构。大型 review 可集中处理跨阶段残留，但不能替代每阶段登记删除门。
- **残留清单是真值点**：阶段残留统一登记在 `plans/` 的专题清单中；聊天中的口头延期不构成备案。清单项目必须包含现状、目标、代码/文档位置、删除或重构条件、验证标准和自然实施顺序；完成后立即删除条目或转入长期 notes。
