...
use crate::budget::{Account, Charge, Taxonomy};
...
pub(crate) struct Slot<T, K: Taxonomy> {
    ...
    suspended: bool,
    pub(crate) _task_charge: Charge<K>,
}

/// 任务表与公平 ready FIFO 的内部算法；公开驱动面见 [`crate::runtime::Runtime`]。
pub(crate) struct WorkQueue<T, K: Taxonomy> {
    tasks: OrderedTable<Slot<T, K>>,
    head: u64,
    tail: u64,
    sealed: bool,
    stop_cursor: u64,
...
impl<T, K: Taxonomy> WorkQueue<T, K> {
    ...
    pub(crate) fn insert(
        ...
        let prepare = (|| {
            ...
            let task_charge = account.acquire_at(task_slot, 1)?;
            Ok((id, task_charge))
        })();
...
impl<T, K: Taxonomy> Drop for WorkQueue<T, K> {
    fn drop(&mut self) {
