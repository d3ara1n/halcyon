//! 观察输出：向内核调试流发消息（测试观测通道）。无颜色无等级——
//! 内核侧统一以 `[pid N]` 话题、debug 等级色印出；要自定义格式/颜色
//! 自己拼进消息字符串。正式的用户态输出是未来的 console 服务。
//!
//! 语义定位：本宏是 stdout 性质的临时观测通道，不随构建档消除
//! （debug/release 均输出）——「release 无 debug 输出」的原则只约束
//! 内核侧日志等级（内核 debug! 已按 debug_assertions 编译期消除），
//! 用户态显式请求的输出没有理由被内核替用户静默。等级分立随未来
//! console 服务一并设计。
//!
